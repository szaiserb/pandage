...
class Coord():
    ...
    def sph2cart(self,sph):
        x = sph['rho'] * cos(sph['elev']) * cos(sph['azim'])
        y = sph['rho'] * cos(sph['elev']) * sin(sph['azim'])
        z = sph['rho'] * sin(sph['elev'])
        cart = {'x':x, 'y':y, 'z':z}
        return cart
    

    def cart2sph(self,cart):
        rho = sqrt(cart['x']**2 + cart['y']**2 + cart['z']**2)
        azim = arctan2(cart['y'],cart['x'])
        elev = arctan2(cart['z'], sqrt(cart['x']**2 + cart['y']**2))
